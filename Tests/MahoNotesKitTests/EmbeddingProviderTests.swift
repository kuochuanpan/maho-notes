...
struct EmbeddingProviderTests {
    ...
    @Test func e5largeModelProperties() {
        let model = EmbeddingModel.e5large
        #expect(model.rawValue == "e5-large")
        #expect(model.huggingFaceId == "intfloat/multilingual-e5-large")
        #expect(model.dimensions == 1024)
        #expect(EmbeddingModel(rawValue: "e5-large") == .e5large)
    }

    @Test func dimensionsPerCase() {
        #expect(EmbeddingModel.minilm.dimensions == 384)
        #expect(EmbeddingModel.e5small.dimensions == 384)
        #expect(EmbeddingModel.e5large.dimensions == 1024)
    }
    ...
    @Test func displayNameForAllModels() {
        ...
        #expect(EmbeddingModel.e5large.displayName == "Multilingual E5 Large")
    }
    ...
    @Test func approximateSizeForAllModels() {
        ...
        #expect(EmbeddingModel.e5large.approximateSize == "~2.2 GB")
    }

    @Test func allCasesIncludesBgeM3() {
        #expect(EmbeddingModel.allCases.count == 3)
        #expect(EmbeddingModel.allCases.contains(.e5large))
    }
}
