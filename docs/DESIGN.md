...
- **Static Site Generator** (for publishing): `unified` / `remark` / `rehype` pipeline with plugins, bundled in MahoNotesKit or as a build step
...
| Markdown | swift-markdown (native), remark/rehype (static site generator) |
| Syntax Highlighting | TreeSitter (native), Shiki (static site) |
